...
static void alarm_remove(cookie_t cookie)
{
  ...
  if ((a = alarm_find(cookie))) {
    ...
    if (a->notification) {
      g_warning(
            "Removing alarm with active notification, please report a bug!");
      g_object_unref(a->notification);
    }

    if (a->has_dbus_filter) {
      g_warning("Removing alarm with active DBUS filter, please report a bug!");
      dbus_connection_remove_filter(system_ui_info->system_bus, dbus_filter,
                                    (gpointer)cookie);
    }
...
static gboolean key_press_cb(GtkWidget *widget, GdkEvent *event,
                             gpointer user_data)
{
  return TRUE;
}
...
static void size_request_cb(GtkWidget *widget, GtkRequisition *requisition,
                            gpointer user_data)
{
  ...
  /* check me */
  height = (2 * (pango_font_metrics_get_descent(metrics) +
...
static gchar *time2str(struct tm *tm)
{
  ...
  if (gconf_client_get_bool(system_ui_info->gc_client,
                            "/apps/clock/time-format", 0))
...
static int alarm_open(const char *interface, const char *method, GArray *param,
                      system_ui_data *sui, system_ui_handler_arg *out)
{
  cookie_t cookie;
...
static int alarm_close(const char *interface, const char *method, GArray *param,
                       system_ui_data *sui, system_ui_handler_arg *out)
{
  system_ui_handler_arg *val;
...
int plugin_init(system_ui_data *sui)
{
  ...
  window_priority =
      gconf_client_get_int(sui->gc_client,
                           "/system/systemui/alarm/window_priority", 0);
...
static void clicked_cb(GtkWidget *widget, gpointer user_data)
{
  ...
  if (a) {
    ...
    if (ae) {
      if (!strcmp(alarm_action_get_label(
                    alarm_event_get_action(ae, a->snooze_action_index)),
                  "cloc_bd_view"))
...
NotifyNotification *alarm_notify_notification_clock(const char *sound_file)
{
  ...
  if (sound_file) {
    ...
    if (plugin) {
      nsv_sv_event_id = nsv_sv_play_event(plugin, 3, sound_file, 1,
                                          "PatternIncomingCall", 1, 100);
      return NULL;
    }
  }
...
static void alarm_notify(cookie_t cookie)
{
  ...
  if ((a = alarm_find(cookie))) {
    ...
    dbus_connection_add_filter(system_ui_info->system_bus, dbus_filter,
                               (void*)cookie, NULL);
    a->has_dbus_filter = TRUE;
    prev_device_orientation = alarm_mce_get_orientation(system_ui_info->system_bus);

    if (is_calendar(a)) {
      gchar *s = gconf_client_get_string(system_ui_info->gc_client,
    ...
    g_free(sound_file);
    a->stop_notification_timeout_tag =
        g_timeout_add_seconds(is_calendar(a) ? 10 : 60, alarm_stop_notification,
                              (gpointer)a->cookie);
...
void alarm_create_clock_alarm_dialog(alarm_event_t *alarm_event)
{
  ...
  gtk_window_set_title(GTK_WINDOW(alarm_dialog),
                       dgettext("osso-clock",
                                "cloc_ti_alarm_notification_title"));
  ...
  if (alarm_event->message && *alarm_event->message) {
    ...
    g_signal_connect_data(GTK_WIDGET(alarm_message_label), "size-request",
                          G_CALLBACK(size_request_cb), 0, 0, 0);
  }
  ...
  if (alarm_event->message && *alarm_event->message) {
    ...
    hildon_helper_set_logical_color(alarm_message_label, GTK_RC_FG, 0,
                                    "DefaultTextColor");
  }
...
void alarm_create_calendar_alarm_dialog(alarm_event_t *alarm_event)
{
  ...
  g_signal_connect_data(label, "size-request", G_CALLBACK(size_request_cb), 0,
                        0, 0);
  gtk_box_pack_start(GTK_BOX(vbox), label, 1, 1, 0);

  gtk_label_set_text(GTK_LABEL(label),
                     alarm_event->title ? alarm_event->title : "");
  label = gtk_label_new(" ");
  gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.5);

  location_label =
      gtk_label_new(alarm_event_get_attr_string(alarm_event, "location", NULL));
  ...
  hildon_helper_set_logical_color(location_label, GTK_RC_FG, 0,
                                  "SecondaryTextColor");

  event_type =
      strdup(alarm_event_get_attr_string(alarm_event, ALARM_CALENDAR_EVENT_TYPE,
                                         "unset"));
  ...
  if (!strcmp(event_type, ALARM_FOR_NORMAL_EVENT)) {
    ...
    if (!strcmp(event_type, ALARM_FOR_ALLDAY)) {
      ...
      if (!strcmp(event_type, ALARM_FOR_TASK)) {
        ...
        icon = gtk_icon_theme_load_icon(icon_theme, "calendar_birthday", 48, 1,
                                        0);
      }
...
gboolean show_alarm_dialog(struct alarm *a)
{
  ...
  if (!alarm_dialog)
  {
    ...
    g_signal_connect_data(alarm_dialog, "delete-event", G_CALLBACK(gtk_true),
                          0, 0, 0);
    g_signal_connect_data(alarm_dialog, "response", G_CALLBACK(response_cb),
                          0, 0, 0);
    g_signal_connect_data(alarm_dialog, "key-press-event",
                          G_CALLBACK(key_press_cb), 0, 0, 0);
    g_signal_connect_data(alarm_dialog, "key-release-event",
                          G_CALLBACK(key_press_cb), 0, 0, 0);
  }
  ...
  for (i=0; i < alarm_event->action_cnt; i++) {
    if (alarm_action_is_button(&alarm_event->action_tab[i])) {
      ...
      const char *textdomain =
          alarm_event_get_attr_string(alarm_event, "textdomain", NULL);

      if( textdomain )
        button = gtk_button_new_with_label(
              dgettext(textdomain, alarm_event->action_tab[i].label));
      else
        button = gtk_button_new_with_label(alarm_event->action_tab[i].label);

      gtk_container_add(GTK_CONTAINER(dialog_button_box), button);

      hildon_gtk_widget_set_theme_size(GTK_WIDGET(button),
...
static void alarm_check_for_powerup()
{
  if (alarm_events_cnt != 1 ||
    ...
    int dlg_result;
    GtkWidget *note =
        hildon_note_new_confirmation(GTK_WINDOW(alarm_dialog),
                                     dgettext("osso-clock",
