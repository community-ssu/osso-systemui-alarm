...
libsystemuiplugin_alarm_dialog.so: osso-systemui-alarm.c
	$(CC) $^ -o $@ -shared $(CFLAGS) $(LDFLAGS) $(shell pkg-config --libs --cflags hildon-1 gconf-2.0 alarm libnotify gtk+-2.0 dbus-1 glib-2.0) -ltime -lhildon-plugins-notify-sv -L/usr/lib/hildon-desktop -Wl,-soname -Wl,$@ -Wl,-rpath -Wl,/usr/lib/hildon-desktop
